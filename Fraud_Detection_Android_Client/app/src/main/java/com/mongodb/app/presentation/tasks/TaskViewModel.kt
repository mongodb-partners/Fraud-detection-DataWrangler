...
class TaskViewModel constructor(
    ...
    val taskListState: SnapshotStateList<Details> = mutableStateListOf()
) : ViewModel() {
    ...
    init {
        viewModelScope.launch {
            repository.getTaskList()
                .collect { event: ResultsChange<Details> ->
                    when (event) {
                        is InitialResults -> {
                            taskListState.clear()
    ...
    fun toggleIsComplete(task: Details) {
    ...
    fun isTaskMine(task: Details): Boolean = repository.isTaskMine(task)
